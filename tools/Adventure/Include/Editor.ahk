...
Sci_Create(n) {
    ...
    Sci[n] := New Scintilla(g_hWndMain, SciX, SciY, SciW, SciH, 0x40000000, 0x200)

    If (g_WordWrap) {
        WinSet Style, -0x100000, % "ahk_id " . Sci[n].hWnd ; -WS_HSCROLL
    }

    Sci[n].SetWrapMode(g_WordWrap)
    Sci[n].SetScrollWidth(-1)
...
CloseTab(TabIndex, Confirm := True, Multiple := False, Exiting := False) {
    Local SkipConfirmation, Result, IsActiveTab, NewIndex, FullPath, hUpDown, Error
    ...
    If (TabEx.GetCount() > 1) {
        ...
        If (IsActiveTab) {
            hUpDown := DllCall("GetWindow", "Ptr", g_hTab, "UInt", 5, "Ptr") ; GW_CHILD
            if (hUpDown) {
                SendMessage 0x472, 0, Error,, ahk_id %hUpDown% ; UDM_GETPOS32
                If (!Error) {
                    SendMessage 0x1330, ErrorLevel - 1,,, ahk_id %g_hTab% ; TCM_SETCURFOCUS
                    Repaint(g_hTab)
                }
            }
...
ShowSyntaxDialog() {
    ...
    Gui Add, Text, x14 y56 w673 h30
    , Select the syntax highlighting scheme of a programming language or document type to be applied to the current document.
    ...
    Gui Add, Button, gSyntaxDlgClose x520 y399 w80 h23, &Cancel
    Gui Add, Button, gSyntaxDlgSubmit vBtnSyntaxApply x606 y399 w80 h23, &Apply
