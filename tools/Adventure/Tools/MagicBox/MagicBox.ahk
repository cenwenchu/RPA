...
IniFile := A_ScriptDir . "\..\..\Settings\MagicBox.ini"
If (!FileExist(IniFile)) {
    If (FileExist(A_AppData . "\AmberSoft\Adventure\MagicBox.ini")) {
        IniFile := A_AppData . "\AmberSoft\Adventure\MagicBox.ini"
    }
}
...
OnWM_KEYDOWN(wParam, lParam, msg, hWnd) {
    ...
    If (wParam == 0x2E) {
        ...
        If (InStr(FocusedControl, "SysL", True)) {
            If (MsgBoxType == "MsgBoxEx") {
...
TDI_GenerateIfBlocks:
    If (cRadioButtons) {
        ...
        Loop %cRadioButtons% {
            ...
            Code .= ") { `; " . RadioButtons[A_Index][2] . "`n`n}"
        }
    ...
    If (cButtons) {
        ...
        Loop %cButtons% {
            ...
            Code .= ") { `; " . StrSplit(CustomButtons[A_Index][2], "`n")[1] . "`n`n}"
        }
    }
    ...
    If (TDI_Buttons || TDI_TimeoutEnabled) {
        ...
        Loop 6 {
            If (TDI_Buttons & (2 ** (A_Index - 1))) {
                ...
                Code .= ") { `; " . TDI_CommonButtons[A_Index][2] . "`n`n} Else "
            }
        }
...
OnWM_ENTERMENULOOP(wParam, lParam, msg, hWnd) {
    Disable := MsgBoxType ~= "^(Task|Msi|MsgBoxEx)"
    DllCall("EnableMenuItem", "Ptr", hSysMenu, "UPtr", 0xDEAD, "UInt", Disable)
    Disable := MsgBoxType ~= "^(MsgBoxEx|Soft|TaskDialogIndirect)"
...
ShowHelp() {
    ...
    , https://www.google.com.br/search?&q=SoftModalMessageBox
    Gui Add, Text, hWndhLink5 vLink5 gOpenURL x24 y258 w450 h18 +0x300
    , https://www.google.com.br/search?&q=MsiMessageBox
