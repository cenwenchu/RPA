...
ShowProperties:
    ...
    SetWindowIcon(hPropWnd, IconLib, -36)
    ...
    Gui Add, Picture, vCtlIcon gBlinkBorder x12 y13 w16 h16 Icon-7, %IconLib%
    ...
    GuiButtonIcon(hBtnReload, IconLib, -50, "L1 T1")
...
OnDropDownChange:
    ...
    If (ClassNN == "Window") {
        GuiControl,, CtlIcon, % "*Icon-7 " . IconLib
...
MenuSetCtlOption:
    ...
    If (MenuItem == "UAC Shield") {
        GoSub SetButtonShield
        Return
    }
...
SetExplorerTheme:
    SetPropertyEx("ExplorerTheme", "Explorer Theme")
Return

SetPropertyEx(Property, DisplayName, NT6orLater := True) {
    If (NT6orLater && !g_NT6orLater) {
        Gui Auto: +OwnDialogs
        MsgBox 0x10, %g_AppName%, This option requires Windows Vista or higher.
        Return
    }

    g[g_Control][Property] := !g[g_Control][Property]
    ...
    If (A_Gui != "Properties") {
        If (g[g_Control].ClassNN == Properties_GetClassNN()) {
            TVCheckOption(hTVCtlOpts, DisplayName, g[g_Control][Property])
        }
...
SetHintText:
    ...
    HintText := InputBoxEx("Hint Text", "A message to be displayed when the " . g[g_Control].Type
    . " control is empty.", g[g_Control].Type . " Property", "Search",,, hAutoWnd,,, IconLib, -38)
...
SetListViewMode:
    ...
    ViewMode := InputBoxEx("View Mode", "Select the ListView viewing mode.", "ListView Property"
    , "Report||List|Icons|Small Icons|Tile", "DDL",, hAutoWnd,,, IconLib, -19)
...
SetButtonShield:
    SendMessage 0x160C, 0, % !g[g_Control].UACShield,, ahk_id %g_Control% ; BCM_SETSHIELD
    SetPropertyEx("UACShield", "UAC Shield")
Return
...
ShowMoreEvents() {
    Run %A_ScriptDir%\..\Constantine\Constantine.ahk /key Windows\GUI\Window\Messages
}
...
IsCtlOptionSet(Option, CtlOpts, hCtl := 0) {
    ...
    If (Option == "UAC Shield") {
        Return g[hCtl].UACShield
    }
...
TVCtlOptsHandler:
    ...
    If (A_GuiEvent == "Normal") {
        ...
        If (ItemText == "UAC Shield") {
            GoTo SetButtonShield
        }
...
SuggestHWndVar:
    hCtl := Properties_GetHandle()
    GuiControl Properties:, EdtHWndVar, % (hCtl != "" ? GenerateVarName(hCtl) : "hWndMain")
Return
